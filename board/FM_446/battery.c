...
static const struct battery_info batt_info_lishen = {
	.voltage_max		= TARGET_WITH_MARGIN(8700, 5), /* mV */
	.voltage_normal		= 6600,
	.voltage_min		= 6000, /* Add 100mV for charger accuracy */
	.precharge_current	= 256,	/* mA */
	.start_charging_min_c	= 0,
	.start_charging_max_c	= 46,
	.charging_min_c		= 10,
...
static int board_get_battery_type(void)
{
	...
	if (1/*!battery_manufacturer_name(name, sizeof(name))*/) {
...
{
	//const struct board_batt_params info[]
	CPRINTS("INFO %d", board_battery_type);
...
enum battery_present battery_hw_present(void)
{
	...
	gpio_FM = gpio_get_level(gpio_bat_present_l);
//	 CPRINTS(" GPIO result : %d", gpio_FM);
	return (gpio_FM ? BP_NO : BP_YES);
}
...
//		CPRINTS("The battery is not present");
		return BP_NO;
	}
//	CPRINTS("The battery is present");
