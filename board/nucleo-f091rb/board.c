...
extern void charger_init_sm5803_5_9(void);
/*
USB_fsco*/
...
void voltage_Batt(void){

	CPRINTS("Voltage bat : %d",battery_get_avg_voltage());
...
int extpower_is_present(void)
{
	...
	CPRINTS("Voltage bat : %d",battery_get_avg_voltage());
	CPRINTS("Current bat : %d",battery_get_avg_current());
	if (board_vbus_source_enabled(CHARGE_PORT_USB_C))
		usb_c_extpower_present = 0;
	else
		usb_c_extpower_present = tcpm_get_vbus_level(CHARGE_PORT_USB_C) ;//tcpc_get_vbus_voltage(0);
