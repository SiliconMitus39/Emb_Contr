...
static int charge_request(int voltage, int current)
{
	...
	if (r2 != EC_SUCCESS)
		{
		//CPRINTS("SONO QUI?  R2 no EC_SUCCESS");
	...
	if (r1 != EC_SUCCESS)
		{
		//CPRINTS("SONO QUI?  R1 no EC_SUCCESS");
	...
	if (voltage > 0 || current > 0) {
		...
		CPRINTS("La tensione %d la corrente %d ", voltage, current);
	} else
		r3 = charger_set_mode(CHARGE_FLAG_INHIBIT_CHARGE);
	if (r3 != EC_SUCCESS)
	{
		//CPRINTS("SONO QUI?  R3 no EC_SUCCESS");
...
void charger_task(void *u)
{
	...
	while (alfa) {
		...
		CPRINTS("CURRENT STATE1:  %d",curr.state);
#ifdef CONFIG_EC_EC_COMM_BATTERY_MASTER
		/*
		...
		if (curr.ac != prev_ac) {
			...
			if (curr.ac) {
				...
				if (rv != EC_SUCCESS) {
					...
					CPRINTS("SONO QUI?  Rv no EC_SUCCESS");
				} else {
				...
				chg_ctl_mode = CHARGE_CONTROL_NORMAL;
				CPRINTS("(^_^) CHARGE_CONTROL_NORMAL %d", chg_ctl_mode);
				battery_seems_to_be_dead = 0;
				prev_ac = curr.ac;
			}
		}
		...
		CPRINTS("Ho richiesto la corrente!! ");
		/* set the battery params*/
		battery_get_params(&curr.batt);
		...
		if (curr.chg.flags & CHG_FLAG_BAD_ANY){
			//CPRINTS("SONO QUI?  RCHG no EC_SUCCESS");
		...
		if (!(curr.batt.flags & BATT_FLAG_RESPONSIVE)) {
			...
			CPRINTS("(^_^) ST_CHARGE");
			CPRINTS("CURRENT STATE:  %d",curr.state);
		}
