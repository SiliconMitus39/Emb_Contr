...
/* Global variable to manage the serial port */
int temp_batt_set = 3010;
int statecharge = 68;
int voltagebatt =7700;
int currbatt = 200;
int desvoltage = 8000;
int descurr = 420;
int  remcap = 1500;
int  fullcap = 3600;
int  status = 0x80;// 0x80, 0x40, 0x20, 0x00  ;
int battflag = 0x01; //BATT_FLAG_WANT_CHARGE;//BATT_FLAG_RESPONSIVE;//
...
void battery_get_params(struct batt_params *batt)
{
	...
	batt_new.desired_voltage = desvoltage ;
	batt_new.desired_current = descurr;
//	batt_new.remaining_capacity = remcap;
//	batt_new.full_capacity = fullcap;
//	batt_new.status  =  status;// 0x80, 0x40, 0x20, 0x00  ;
//batt_new.flags = BATT_FLAG_WANT_CHARGE;//BATT_FLAG_RESPONSIVE;//
//CPRINTS("BATT_FLAG_1 %d",batt_new.flags  );
...
//		batt_new.flags |= BATT_FLAG_BAD_DESIRED_CURRENT;
...
#endif
	{
		...
		CPRINTS("BATT_FLAG_3 %d",batt_new.flags  );
	}
	else
	{	/* Force both to zero */
		batt_new.desired_voltage = batt_new.desired_current = 0;
		CPRINTS("BATT_FLAG_2 %d",batt_new.flags  );
	}
